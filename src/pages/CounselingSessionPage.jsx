...
export default function CounselingSessionPage() {
  ...
  async function loadAll() {
    // session
    const { data: s, error: e1 } = await supabase.from("counsel_sessions").select("*").eq("id", sessionId).single();
    if (e1) return alert("상담 로드 실패: " + e1.message);
    setSession(s);

    // type
    const { data: t, error: e2 } = await supabase.from("counsel_test_types").select("*").eq("id", s.test_type_id).single();
    ...
    // scores
    const { data: sc, error: e4 } = await supabase.from("counsel_session_scores").select("*").eq("session_id", sessionId);
  ...
  async function saveAll() {
    ...
    try {
      ...
      // comments sync (delete + insert)
      const { error: e3 } = await supabase.from("counsel_session_comment_map").delete().eq("session_id", sessionId);
      if (e3) throw e3;
    ...
    <div style={wrap}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "flex-start" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
          ...
          <button style={btn} onClick={saveAll} disabled={saving}>{saving ? "저장중…" : "저장"}</button>
          <button style={btn} onClick={downloadPdf} disabled={pdfBusy}>PDF 다운로드</button>
          <button style={btn} onClick={uploadPdfToStorage} disabled={pdfBusy}>PDF 저장(서버)</button>
        </div>
      </div>
      ...
      <div style={{ marginTop: 14, borderTop: `1px solid ${COLORS.lineSoft}`, paddingTop: 14 }}>
        ...
        <div style={{ marginTop: 10 }}>
          <textarea
            value={session.overall_note || ""}
            onChange={(e) => setSession((p) => ({ ...p, overall_note: e.target.value }))}
            placeholder="총평(자유 코멘트)"
            style={{
              width: "100%",
              minHeight: 90,
              padding: "10px 12px",
      ...
      <div style={{ marginTop: 18, borderTop: `1px solid ${COLORS.lineSoft}`, paddingTop: 14 }}>
        ...
        >
          {/* 상단 헤더 */}
          <div style={{ fontSize: 18, fontWeight: 900, marginBottom: 10 }}>산본 블라썸에듀 · 레벨테스트 상담결과</div>
          ...
          <div style={{ borderTop: `1px solid ${COLORS.lineSoft}`, paddingTop: 10, marginTop: 10 }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {areas.map((a) => (
                  <tr key={a.id}>
                    <td style={{ padding: "8px 6px", borderTop: `1px solid ${COLORS.lineSoft}`, fontWeight: 900 }}>{a.name}</td>
              ...
              return (
                <div key={a.id} style={{ marginBottom: 10 }}>
                  ...
                  <ul style={{ marginTop: 6, marginBottom: 0, paddingLeft: 18 }}>
                    {list.map((t) => (
                      <li key={t.id} style={{ marginBottom: 4, lineHeight: 1.35 }}>{t.content}</li>
                    ))}
                  </ul>
                </div>
              );
          ...
          {/* 총평 */}
          <div style={{ borderTop: `1px solid ${COLORS.lineSoft}`, paddingTop: 10, marginTop: 10 }}>
            <div style={{ fontWeight: 900, marginBottom: 6 }}>총평</div>
            <div style={{ whiteSpace: "pre-wrap", lineHeight: 1.45, color: COLORS.sub }}>
              {session.overall_note?.trim() ? session.overall_note : "-"}
            </div>
          </div>

          <div style={{ marginTop: 14, paddingTop: 10, borderTop: `1px solid ${COLORS.lineSoft}`, color: COLORS.sub }}>
            ※ 산본 블라썸에듀
          </div>
        </div>
      </div>
